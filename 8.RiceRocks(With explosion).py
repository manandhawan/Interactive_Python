...
def group_collide(group, other_object):
    ...
    for x in list(group):
        if x.collide(other_object):
            group.remove(x)
            explosion_group.add(Sprite(x.pos,x.vel,0,0,explosion_image,explosion_info))
...
def group_group_collide(group1,group2):
    global score,last_game_score
    collisions1 = 0
    for x in list(group1):
        for y in list(group2):
            collisions = group_collide(group2,x)
            if x.collide(y):
                group1.remove(x)
...
def draw(canvas):
    ...
    score_label.set_text(str(last_game_score))
...
def rock_spawner():
    ...
    if score > 400:
        rock_vel = [random.randrange(0,5), random.randrange(0,5)]
...
label = frame.add_label(" RICEROCKS ")
space = frame.add_label("")
label = frame.add_label("- Controls:")
label = frame.add_label("* Up --> Thrusters")
label = frame.add_label("* Left/Right --> Spin Control")
label = frame.add_label("* Space --> Fire Weapon")
space = frame.add_label("")
